...
class PicUpload {
	...
	function _makeFilename() {
		...
		$this->_filePath= $this->_uploadPath.date('y/m/d').'/'.$this->_fileData['name'];
	}
	function upload() {
		$this->_makeFilename();
		...
		if(move_uploaded_file(stripQuotes($this->_fileData['tmp_name']), $this->_filePath)){
           return str_replace($this->_uploadPath,'',$this->_filePath);
		}
	}
