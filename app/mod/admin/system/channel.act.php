...
class Channel extends Page{
	...
	function listNode(){
		...
		$sql = $pvar['kw']['kw_expan']
			? "select m1.id as id, m1.title as title, m1.lev as lev, m1.child_num as child_num, m1.description as description from ".$this->tab." m1,".$this->tab." m2 where  m1.lft between m2.lft and m2.rgt and m2.id=".$pvar['kw']['kw_pid']." and m1.id <>1 and m1.id <> {$pvar['kw']['kw_pid']} order by m1.lft"
		...
		$pvar['title'] = '网站栏目';
		$pvar['btns'] = $this->btnAddNode(1,'添加主栏目').$this->btnAddNode($pvar['kw']['kw_pid'],'在当前位置添加栏目');
	...
	function move(){
		...
		if (!strlen($this->input['id'])) {
			Core::raiseMsg('错误! 没有指定栏目ID号!');
		}
		if(('up' == $direction) || ('down' == $direction)){
			$this->node->moveNode($this->input['id'], $direction);
		}
		else Core::raiseMsg('错误!方向参数不正确,无法执行移动栏目位置的操作...');
		Core::redirect(Core::getUrl('','','',true));
	}
	function remount(){
	...
	function btnAddNode($pid,$label){
		$url = Core::getUrl('','',array('act'=>'add','pid'=>$pid));
		return " <input type='button' onclick=\"location.replace('{$url}');\" value='$label' />";
	}
	function btnAdd($pid){
		$url = Core::getUrl('','',array('act'=>'add','pid'=>$pid));
		return " <a href='#' onclick=\"location.replace('$url');\"><img src='imgs/icon/ins.gif' title='增加子栏目' alt='' align='absmiddle' border='0' /></a>";
	}
	function btnDel($id){
		$msg = '在执行此操作时会把当前栏目的所有子栏目同时删除,你确定要删除吗？';
	...
	function btnUpdate($id){
		$url = Core::getUrl('','',array('act'=>'update','id'=>$id));
	...
	function btnRemount($id){
		$msg = "你正在操作ID号为‘".$id."’的栏目,请输入你要挂接到的栏目ID号:";
		$url = Core::getUrl('','',array('act'=>'remount','id'=>$id));
		return " <a href='#' onclick=\"if(n = prompt('{$msg}'))location.replace('{$url}'+',pid,'+n);\"><img src='imgs/icon/tree.gif' title='改变父栏目' alt='' align='absmiddle' border='0' /></a>";
	}
	function btnMoveUp($id){
		$url = Core::getUrl('','',array('act'=>'move','id'=>$id,'dir'=>'up'));
	...
	function btnMoveDown($id){
		$url = Core::getUrl('','',array('act'=>'move','id'=>$id,'dir'=>'down'));
