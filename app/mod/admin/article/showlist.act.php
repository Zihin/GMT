...
class ShowList extends Page{
	...
	function process(){
		...
		if(is_numeric($this->input['kw_istop']))
			$pvar['kw']['kw_istop'] = $this->input['kw_istop'];
		...
		$sqlWhere .= strlen($pvar['kw']['kw_istop']) ? " and i.istop = '{$pvar['kw']['kw_istop']}'" : '';
		$sqlWhere .= $pvar['kw']['kw_title'] ? " and i.title like('%{$pvar['kw']['kw_title']}%')" : '';
		$sqlWhere .= $pvar['kw']['kw_cate'] ? " and i.cate in (".implode(Category::getAllChild($pvar['kw']['kw_cate']),',').")" : " and i.cate in (".implode(Category::getAllChild(CATE_INFO),',').")";
		...
		$sql = "select i.id as id, i.state as state, i.title as title, i.put_time as put_time, i.istop as istop, u.username as editor,c.title_zh as category from "
			.$this->tab." i left join ".TB_PREFIX."sys_user u on i.editor=u.id
                          left join ".TB_PREFIX."category c on c.id=i.cate";
		...
		while(!$rs->EOF){
			...
			$rs->fields['istop'] = $rs->fields['istop']?'<span style="color:red">【置顶】</span>':'';
		...
		$pvar['istop_sel'] = Form::select('kw_istop',array(''=>'', '0'=>'未置顶', '1'=>'置顶'),$pvar['kw']['kw_istop']);
