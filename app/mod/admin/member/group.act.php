...
class Group extends Page {
	...
	function permsSet() {
		if (!is_numeric($this->input['gid'])) {
			Core :: raiseMsg('错误!没有指定会员组ID号');
		}
		if (empty ($this->input['submit'])) {
			$this->showSettingForm($this->input['gid']);
			return;
		}
		$sql= " delete from ".TB_PREFIX."mb_group_priv where group_id =".$this->input['gid'];
		$this->db->Execute($sql);

		if (is_array($this->input['id'])) {
			foreach ($this->input['id'] as $v) {
				$value= "(".$this->input['gid'].",$v)";
				$sql= "insert into ".TB_PREFIX."mb_group_priv values $value;";
				$this->db->Execute($sql);
			}
	...
	function getAllPerms($gid) {
		$sql= "select action_id from ".TB_PREFIX."mb_group_priv where group_id=$gid";
		$rs= $this->db->Execute($sql);
		while (!$rs->EOF) {
			$aid[]= $rs->fields['action_id'];
			$rs->MoveNext();
		}

		$sql= "select * from ".TB_PREFIX."mb_permissions order by title";
		$rs= $this->db->Execute($sql);
		while (!$rs->EOF) {
			$rs->fields['check']= is_array($aid) && in_array($rs->fields['id'], $aid) ? 'checked=checked' : '';
			$list[]= $rs->fields;
			$rs->MoveNext();
		}
		return $list;
	...
	function btnAdd() {
		...
		return " <input type='button' onclick=\"location.replace('{$url}');\" value='新建会员组' />";
	}
