...
class Login extends Page{
	...
	function process(){
		...
		$this->sess->set('groupname',$this->db->GetOne("select title from ".TB_PREFIX."sys_group where id={$userinfo['gid']}"));
		
		$this->sess->setUserId($userinfo['id']);
		$this->sess->setGroupId($userinfo['gid']);
		
		//跳转页面
		Core::redirect(Core::getUrl('index','default'));
	}
	...
	function showForm($data,$eMsg=null){
		...
		$pvar['dyncode_img'] = Core::getUrl('dyncodeImg');
	...
	function checkInput(& $data){
		$eMsg = array();
		if(GD_ENABLE && $this->sess->get('login_times')>2 && $data['dyncode'] != $this->sess->get('dyncode')){
	...
	function validate($userinfo){
		...
		$sql = "select count(*) from ".TB_PREFIX."sys_group where id=".(int)$userinfo['gid'];
		if(!$this->db->getOne($sql)){
			$eMsg['msg'] = renderMsg('你所属的用户组不存在,无法为你分配权限,请与管理员联系!');
			return $eMsg;
		}
		$this->sess->set('login_times');	//清空尝试登录的次数
		return;
	}
	...
	function getUserInfo($usernmae){
		$sql = "select * from ".TB_PREFIX."sys_user where username = '$usernmae'";
